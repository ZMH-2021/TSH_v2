...
int CacheSetCount(CacheConn *cache_conn, string key, int64_t count)
{
    ...
    snprintf(in_buffer, BUFFER_SIZE, "SET %s %d", key, count);
    char out_buffer[BUFFER_SIZE] = {0};

    if (kvstore_send_command(in_buffer, out_buffer) != 0)
    {
        return -1;
    }
    return 0;
...
int CacheGetCount(CacheConn *cache_conn, string key, int64_t &count)
{
    count = 0;
    char in_buffer[BUFFER_SIZE] = {0};
    snprintf(in_buffer, BUFFER_SIZE, "GET %s", key);
    char out_buffer[BUFFER_SIZE] = {0};
...
{
    int64_t count = 0;
    CacheGetCount(cache_conn, key, count);

    char in_buffer[BUFFER_SIZE] = {0};
    snprintf(in_buffer, BUFFER_SIZE, "SET %s %d", key, count + 1);
    char out_buffer[BUFFER_SIZE] = {0};
